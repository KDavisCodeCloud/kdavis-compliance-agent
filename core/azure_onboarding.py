...
Identical shape to kdavis-finops-agent's module of the same name (copied,
not shared -- separate repo, same convention as core/aws_onboarding.py).
Azure's built-in `Reader` + `Cost Management Reader` roles already cover
everything AzureProvider.collect() needs -- no custom JSON policy to
hand-build like AWS's build_permissions_policy().
"""
...
def generate_setup_instructions(subscription_id_placeholder: str = "<your-subscription-id>") -> str:
    ...
    tenant, to create the Service Principal this service will use. No
    per-tenant value (like AWS's external_id) is baked into these
    instructions, so this function takes no tenant-specific arguments."""
    roles = " and ".join(f'"{r}"' for r in _REQUIRED_ROLES)
    return (
        "In Azure: run `az ad sp create-for-rbac --name compliance-agent-readonly "
