...
class TestVerifyRole:
    def test_raises_clear_error_on_failure(self):
        with patch("boto3.client") as mock_client:
            ...
            with pytest.raises(AssumeRoleError):
                verify_role("arn:aws:iam::222222222222:role/compliance", "ext-abc")
    ...
    def test_succeeds_silently_when_role_is_assumable(self):
        with patch("boto3.client") as mock_client:
            ...
            verify_role("arn:aws:iam::222222222222:role/compliance", "ext-abc")  # no raise
...
class TestAssumeRoleSession:
    def test_builds_a_session_from_the_temp_credentials(self):
        with patch("boto3.client") as mock_client, patch("boto3.Session") as mock_session:
            ...
            assume_role_session("arn:aws:iam::222222222222:role/compliance", "ext-abc")

            # A session built from raw assumed-role creds has no region behind
            # it (same bug class fixed live in kdavis-finops-agent) -- must be
            # set explicitly.
