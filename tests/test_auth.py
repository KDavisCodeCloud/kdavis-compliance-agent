...
not a hand-built fake dict. Every route-level test in this repo passes
its own fake tenant dict directly to the route function, bypassing this
query entirely -- which is exactly how the Azure columns could go
missing from this SELECT while every other test still passed. This file
exists specifically to catch that class of bug (mirrors kdavis-finops-
agent's test_auth.py of the same name and purpose).
"""
...
class TestGetTenant:
    ...
    async def test_select_statement_names_every_required_column(self):
        ...
        for column in _REQUIRED_KEYS - {"id"}:
